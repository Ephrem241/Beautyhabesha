...
export function PaymentRecordsTable({ payments: initialPayments }: PaymentRecordsTableProps) {
  ...
  const handleAction = (
    ...
    startTransition(async () => {
      ...
      const result: PaymentActionResult =
        action === "approve"
          ? await approvePaymentRecord({ ok: true }, formData)
          : await rejectPaymentRecord({ ok: true }, formData);
          ...
          <tbody>
            {items.map((payment) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  {payment.amount.toLocaleString("en-ET")} ETB
                </td>
          ...
          {items.map((payment) => (
            ...
            >
              ...
              <div className="mt-4 flex gap-2">
                ...
                >
                  {isPending && pendingId === payment.id ? "Working..." : "Approve"}
                </Button>
                <Button
                  type="button"
                  disabled={isPending && pendingId === payment.id}
